...
shell('DELIVERY ECONOMICS','Turn obligations into visibility.','Trace contract value into recognized revenue and remaining backlog, with transparent currency conversion.','amber')
df=load_data()
...
nonempty(f)
s=summary(f)
metrics([('Contracted value',money(s['contract_value'])),('Recognized',money(s['recognized'])),('Remaining obligation',money(s['backlog'])),('Projects',str(s['projects']))])
a,b=st.columns([1.7,1])
with a:
    reg=f.groupby('region',as_index=False)[['recognized_revenue_usd','backlog_usd']].sum()
    chart(px.bar(reg,x='region',y=['recognized_revenue_usd','backlog_usd'],title='Recognized versus remaining',barmode='stack'),400)
with b:
    st.subheader('Delivery mix')
    mix=f.groupby('project_type').backlog_usd.sum().sort_values(ascending=False)
    for label,v in mix.items():
        st.write(f'{label} · **{money(v)}**')
        st.progress(float(v/max(mix.sum(),1)))
project_tab,fx_tab=st.tabs(['Project drilldown','Currency controls'])
with project_tab:
    project=st.selectbox('Project',f.sort_values('backlog_usd',ascending=False).project_id.tolist())
    r=f[f.project_id==project].iloc[0]
    brief(f'{project}: {money(r.contract_value_usd)} contracted, {money(r.recognized_revenue_usd)} recognized, {money(r.backlog_usd)} remaining. All display totals are USD.')
    table(f.sort_values('backlog_usd',ascending=False),'backlog_detail')
with fx_tab:
    table(f[['currency','fx_to_usd']].drop_duplicates(),'fx_assumptions',220)
    st.caption('Fixed illustrative FX rates, not live market rates. USD = local-currency amount × FX rate. Project classifications are synthetic labels, not accounting-policy determinations.')
    difference=(f.contract_value_usd-f.recognized_revenue_usd-f.backlog_usd).abs().max()
    st.success(f'Contract value reconciles to recognized plus backlog. Maximum row difference: ${difference:.6f}.')
